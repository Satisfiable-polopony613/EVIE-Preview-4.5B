...
On the first run this downloads the 22 public ViDoRe datasets into `./vidore` (about 55 GB): the ten [ViDoRe V1](https://huggingface.co/collections/vidore/vidore-benchmark-667173f98e70a1c0fa4db00d) sets, the four [ViDoRe V2](https://huggingface.co/collections/vidore/vidore-benchmark-v2) sets, and the eight public [ViDoRe V3](https://huggingface.co/collections/vidore/vidore-benchmark-v3) domains. Pass a path to reuse an existing copy: `bash reproduce.sh /path/to/vidore`.

The run uses every visible GPU and writes a timestamped log. It prints one line per task and ends with the four aggregates:
